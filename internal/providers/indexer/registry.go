...
// Factory is a constructor function that returns a new, unconfigured Indexer.
type Factory func() Indexer

// Registry holds the set of known indexer factories keyed by implementation
// name (e.g. "Newznab").
...
func (r *Registry) Register(name string, f Factory) {
	if _, exists := r.factories[name]; exists {
		panic(fmt.Sprintf("indexer: duplicate registration for %q", name))
	}
	r.factories[name] = f
}
...
func (r *Registry) Get(name string) (Factory, error) {
	...
	if !ok {
		return nil, fmt.Errorf("indexer: no factory registered for %q", name)
	}
	return f, nil
}
