...
func (q *Queries) ListEpisodeFilesForSeries(ctx context.Context, seriesID int64) ([]EpisodeFile, error) {
	rows, err := q.db.QueryContext(ctx, listEpisodeFilesForSeries, seriesID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
