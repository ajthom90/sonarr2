// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: root_folders.sql

package postgres

import (
	"context"
)

const createRootFolder = `-- name: CreateRootFolder :one
INSERT INTO root_folders (path)
VALUES ($1)
RETURNING id, path, created_at
`

func (q *Queries) CreateRootFolder(ctx context.Context, path string) (RootFolder, error) {
	row := q.db.QueryRow(ctx, createRootFolder, path)
	var i RootFolder
	err := row.Scan(&i.ID, &i.Path, &i.CreatedAt)
	return i, err
}

const deleteRootFolder = `-- name: DeleteRootFolder :exec
DELETE FROM root_folders WHERE id = $1
`

func (q *Queries) DeleteRootFolder(ctx context.Context, id int32) error {
	_, err := q.db.Exec(ctx, deleteRootFolder, id)
	return err
}

const getRootFolder = `-- name: GetRootFolder :one
SELECT id, path, created_at
FROM root_folders
WHERE id = $1
`

func (q *Queries) GetRootFolder(ctx context.Context, id int32) (RootFolder, error) {
	row := q.db.QueryRow(ctx, getRootFolder, id)
	var i RootFolder
	err := row.Scan(&i.ID, &i.Path, &i.CreatedAt)
	return i, err
}

const getRootFolderByPath = `-- name: GetRootFolderByPath :one
SELECT id, path, created_at
FROM root_folders
WHERE path = $1
`

func (q *Queries) GetRootFolderByPath(ctx context.Context, path string) (RootFolder, error) {
	row := q.db.QueryRow(ctx, getRootFolderByPath, path)
	var i RootFolder
	err := row.Scan(&i.ID, &i.Path, &i.CreatedAt)
	return i, err
}

const listRootFolders = `-- name: ListRootFolders :many
SELECT id, path, created_at
FROM root_folders
ORDER BY path
`

func (q *Queries) ListRootFolders(ctx context.Context) ([]RootFolder, error) {
	rows, err := q.db.Query(ctx, listRootFolders)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []RootFolder{}
	for rows.Next() {
		var i RootFolder
		if err := rows.Scan(&i.ID, &i.Path, &i.CreatedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
