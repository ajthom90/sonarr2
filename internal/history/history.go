...
// It provides a HistoryStore interface with Postgres and SQLite implementations
...
// HistoryEntry is a single grab/import/failure event for an episode.
...
// HistoryStore provides read/write access to history entries.
type HistoryStore interface {
	// Create persists a new history entry and returns it with the assigned ID
	// and server-generated date.
	Create(ctx context.Context, entry HistoryEntry) (HistoryEntry, error)

	// ListForSeries returns all history entries for the given series, ordered
	// by date descending.
	ListForSeries(ctx context.Context, seriesID int64) ([]HistoryEntry, error)

	// ListForEpisode returns all history entries for the given episode,
	// ordered by date descending.
	ListForEpisode(ctx context.Context, episodeID int64) ([]HistoryEntry, error)

	// FindByDownloadID returns history entries whose download_id matches,
	// ordered by date descending.
	FindByDownloadID(ctx context.Context, downloadID string) ([]HistoryEntry, error)
