...
// commandResource is the v6 JSON shape for a command.
...
// commandEnqueueInput is the JSON body for POST /api/v6/command.
...
func (h *commandHandler) enqueue(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Error("v6 command enqueue", slog.String("err", err.Error()))
...
func (h *commandHandler) list(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Error("v6 command list", slog.String("err", err.Error()))
		WriteError(w, r, http.StatusInternalServerError, "Internal Server Error")
		return
	}
	if cmds == nil {
...
func (h *commandHandler) get(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, commands.ErrNotFound) {
		WriteNotFound(w, r, fmt.Sprintf("No command with id %d", id))
		return
	}
	if err != nil {
		h.log.Error("v6 command get", slog.String("err", err.Error()))
		WriteError(w, r, http.StatusInternalServerError, "Internal Server Error")
		return
	}
